...
/**
 *   	\file       depensefixe_list.php
 *		\ingroup    tresoreriemensuelle
 *		\brief      List page for depensefixe
 */
...
require_once __DIR__.'/class/depensefixe.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('tresoreriemensuelle', 'depensefixe', 'write');
	$permissiontodelete = $user->hasRight('tresoreriemensuelle', 'depensefixe', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("tresoreriemensuelle")) {
	accessforbidden('Module tresoreriemensuelle not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DepenseFixe';
	$objectlabel = 'DepenseFixe';
	$uploaddir = $conf->tresoreriemensuelle->dir_output;
...
$title = $langs->trans("DepenseFixes");
//$help_url = "EN:Module_DepenseFixe|FR:Module_DepenseFixe_FR|ES:Módulo_DepenseFixe";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/tresoreriemensuelle/depensefixe_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-tresoreriemensuelle page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/tresoreriemensuelle/depensefixe_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDepenseFixeRef";
$modelmail = "depensefixe";
$objecttmp = new DepenseFixe($db);
