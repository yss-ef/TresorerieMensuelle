...
/**
 * \file        class/depensefixe.class.php
 * \ingroup     tresoreriemensuelle
 * \brief       This file is a CRUD class file for DepenseFixe (Create/Read/Update/Delete)
 */
...
/**
 * Class for DepenseFixe
 */
class DepenseFixe extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'tresoreriemensuelle';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'depensefixe';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'tresoreriemensuelle_depensefixe';

	/**
	 * @var string 	If permission must be checkec with hasRight('tresoreriemensuelle', 'read') and not hasright('mymodyle', 'depensefixe', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'tresoreriemensuelle';

	/**
	 * @var string 	String with name of icon for depensefixe. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'depensefixe@tresoreriemensuelle' if picto is file 'img/object_depensefixe.png'.
	 */
	...
	public $fields = array(
		...
		"day_of_month" => array("type" => "integer", "label" => "day_of_month", "enabled" => "1", 'position' => 0, 'notnull' => 0, "visible" => "1", "default" => "1",),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'tresoreriemensuelle_depensefixeline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_depensefixe';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'DepenseFixeline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'DepenseFixe', 'fk_element'=>'fk_depensefixe'));
	...
	//  */
	// protected $childtablesoncascade = array('tresoreriemensuelle_depensefixedet');

	// /**
	//  * @var DepenseFixeLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('tresoreriemensuelle', 'depensefixe', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('tresoreriemensuelle', 'depensefixe', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('tresoreriemensuelle', 'depensefixe_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'depensefixe/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'depensefixe/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'depensefixe/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'depensefixe/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->tresoreriemensuelle->dir_output.'/depensefixe/'.$oldref;
				$dirdest = $conf->tresoreriemensuelle->dir_output.'/depensefixe/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->tresoreriemensuelle->dir_output.'/depensefixe/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('tresoreriemensuelle','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('tresoreriemensuelle','tresoreriemensuelle_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'TRESORERIEMENSUELLE_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'TRESORERIEMENSUELLE_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'TRESORERIEMENSUELLE_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDepenseFixe")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DepenseFixe").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/tresoreriemensuelle/depensefixe_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDepenseFixe");
	...
	public function LibStatut($status, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			global $langs;
			//$langs->load("tresoreriemensuelle@tresoreriemensuelle");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DepenseFixeLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_depensefixe:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('TRESORERIEMENSUELLE_MYOBJECT_ADDON')) {
			$conf->global->TRESORERIEMENSUELLE_MYOBJECT_ADDON = 'mod_depensefixe_standard';
		}

		if (getDolGlobalString('TRESORERIEMENSUELLE_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('TRESORERIEMENSUELLE_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('TRESORERIEMENSUELLE_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/tresoreriemensuelle/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDepenseFixe $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/tresoreriemensuelle/doc/";
...
/**
 * Class DepenseFixeLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DepenseFixeLine extends CommonObjectLine
{
	// To complete with content of an object DepenseFixeLine
	// We should have a field rowid, fk_depensefixe and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'depensefixe'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_depensefixe'
